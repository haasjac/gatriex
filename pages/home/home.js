...
function setEventHandlers() {
    ...
    $("#searchButton").click(function () {
       searchName = $("#searchSummonerName").val().split(" ").join("");
       clearSummonerData();
       getSummoner();
    });
}
    ...
    for (var serviceIndex = 0; serviceIndex < services.length; serviceIndex++) {
        ...
        for (var incidentIndex = 0; incidentIndex < incidents.length; incidentIndex++) {
            if (incidents[incidentIndex].active) {
                ...
                var severity = '<i class="fa fa-li ' + (updates[0].severity == "info" ? "fa-info-circle" : "fa-question-circle") + '" data-severity="' + updates[0].severity + '"></i>';
